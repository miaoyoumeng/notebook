...
> 参考：https://pdai.tech/md/db/sql-mysql/sql-mysql-mvcc.html
...
* DATA_TRX_ID（6字节）：记录最近更新这条记录的事务ID，如果这行数据第一次事务操作，则该列为空（比如这行数据首次insert）。
* DATA_ROLL_PTR（7字节）：指向该行undo log的指针，通过指针找到之前版本，通过链表形式组织(7字节)
* DB_ROW_ID（6字节）：隐含的自增ID（隐藏主键），行标识，没有主键时主动生成。如果有主键，则不会有该隐藏列。
* DELETED_BIT（1字节）：记录被更新或删除并不代表真的删除，而是删除flag变了。

![image](../imgs/db-mysql-mvcc-1.png)

如上图，DB_ROW_ID是数据库默认为该行记录生成的唯一隐式主键；<font color="red">表有主键MySQL不会增加一个DB_ROW_ID隐式主键</font>。DB_TRX_ID是当前操作该记录的事务ID； 而DB_ROLL_PTR是指向这行记录上一个版本的指针值，用于配合undo日志，指向上一个旧版本；由于改记录上一个版本的数据存储在undo log中，故该值是指向undo log一个指针值。delete flag没有展示出来。
...
* insert undo log：插入一条记录时，至少要把这条记录的主键值记下来， 代表事务在insert新记录时产生的undo log，只在事务回滚时需要；在事务回滚时，只需要把这个主键值对应的记录删掉就好了。并且在事务提交后可以被立即丢弃该日志。
...
![alt 图表](../imgs/insert_undo_log.png)
...
![alt 图表](../imgs/update_undo_log.png)
