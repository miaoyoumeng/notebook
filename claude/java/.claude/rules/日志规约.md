...
@Slf4j
public class LogKlass {
    
    // ✅ debug 使用需要先判断日志级别，再输出日志
    public void debugMethod() {
        if (log.isDebugEnabled()) {
             log.debug("fetch_market slug={}", slug);
        }
    }

    // ✅ info 使用需要先判断日志级别，再输出日志
    ...
    public void warnMethod() {
        log.warn("fetch_market warn slug={}", slug);
    }


    // ✅ error 则直接输出日志
    public void errorMethod() {
        log.error("failed_fetch_market slug={}", slug, exception);
    }
}
