...
从源码可知，其底层是由AQS提供支持，所以其数据结构可以参考AQS的数据结构，而AQS的数据结构核心就是两个虚拟队列: 同步队列sync queue
和条件队列condition
...
说明:
由源码可知，对CountDownLatch对象的await的调用会转发为对Sync的acquireSharedInterruptibly(从AQS继承的方法)方法的调用。
...
说明:
从源码中可知，acquireSharedInterruptibly又调用了CountDownLatch的内部类Sync的tryAcquireShared和AQS的doAcquireSharedInterruptibly函数。
...
![](../../imgs/concurrent/java-thread-x-countdownlatch-1.png)
...
![](../../imgs/concurrent/java-thread-x-countdownlatch-2.png)
...
![](../../imgs/concurrent/java-thread-x-countdownlatch-3.png)
...
![](../../imgs/concurrent/java-thread-x-countdownlatch-4.png)
...
![](../../imgs/concurrent/java-thread-x-countdownlatch-5.png)

说明: 此时，Sync queue队列里的结点个数未发生变化，但是此时，AQS的state已经变为1了。

  * t2线程执行countDownLatch.countDown操作，主要调用的函数如下。

![](../../imgs/concurrent/java-thread-x-countdownlatch-6.png)
...
![](../../imgs/concurrent/java-thread-x-countdownlatch-7.png)
