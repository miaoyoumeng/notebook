#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""prd-writer 确定性检查脚本（继承自原 prd-validator）。
...
用法：
    python3 markdown-formatter.py <PRD 文件或目录> [...]
    # 目录输入递归扫描 *.md
...
无第三方依赖。严重度与 prd-writer/SKILL.md「验证（Verification）- 公共规则 - 严重程度」一致。
...
def main():
    args = sys.argv[1:]
    if not args:
        print("用法: uv run python markdown-formatter.py <PRD 文件或目录> [...]", file=sys.stderr)
        sys.exit(2)
