...
- 读文件：只能读目录 `references/user-story`、 `outputs/prompts/`、`outputs/prds/` 、`outputs/todos/` 和 `logs`， 禁止读其他目录。
...
- 按字母顺序列出 `outputs/prompts/*.prompt.md`，与`logs/prompts-progress.txt`内容对比缺失文档。
...
- 再按字母顺序读取 `outputs/prompts/*.prompt.md` 内容。
...
- 先执行命令 `/solo:commander-prompt-runner init`, 将缺失的文档添加到`logs/prompts-progress.txt`中。
- 再按字母顺序列出 `outputs/prompts/*.prompt.md`，分组依次显示`🟡 待处理`/`🔴 处理失败`列表。
- 然后停止
...
- 依次备份`outputs/prompts/*.prompt.md`中对应名称的文件，备份目录为 `backup/prompts`。
- 确认备份完毕，没有遗漏。
