...
| 原则 | 含义 | 说明 |
|------|------|------|
| **A**utomatic | 自动化 | 全自动执行、非交互式、用 assert 验收、不准用 System.out |
| **I**ndependent | 独立性 | 测试之间不互相调用、不依赖执行先后顺序 |
...
| 原则 | 说明 | 示例 |
|------|------|------|
| **B**order | 边界值测试 | 循环边界、特殊取值（null/空字符串/0）、特殊时间点、数据顺序 |
| **C**orrect | 正确输入 | 正常输入得到预期结果 |
| **D**esign | 设计文档 | 与设计文档/接口契约对齐编写测试 |
| **E**rror | 错误输入 | 非法数据、异常流程、非业务允许输入，验证预期错误 |
