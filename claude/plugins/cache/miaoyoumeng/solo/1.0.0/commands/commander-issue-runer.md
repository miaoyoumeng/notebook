...
- 读文件：只能读目录 `<当前工作目录>/outputs/prds/` 、`<当前工作目录>/outputs/issues/` 和 `<当前工作目录>/logs`， 禁止读其他目录。
...
- 按字母顺序列出 `<当前工作目录>/outputs/issues/**/feature-*.md`，与`<当前工作目录>/logs/issues-progress.txt`内容对比缺失文档。
- 将缺失的文档，按照文档名称字典序插入对应的位置，并将状态设置为`[文件名] 🟡 待处理` 。
- 再按字母顺序读取 `<当前工作目录>/outputs/issues/**/feature-*.md` 内容。
...
- 全部问题和方案确认完毕后，更新文档中约定的文档路径中，禁止直接更新`<当前工作目录>/outputs/prds`下的 prd 文档。
...
- 依次备份`<当前工作目录>/outputs/issues/**/*.md`中对应名称的文件，备份目录为 `<当前工作目录>/backup/issues`。
- 确认备份完毕，没有遗漏。
